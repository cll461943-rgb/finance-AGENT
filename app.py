...
with st.sidebar:
    ...
    provider = "dashscope"
    st.caption("LLM: DashScope / DeepSeek V4 Pro")
    dashscope_key_from_env = bool(os.environ.get("DASHSCOPE_API_KEY"))
    if dashscope_key_from_env:
        st.success("DASHSCOPE_API_KEY loaded from local environment. It is hidden.")
        provider_key = ""
    else:
        provider_key = st.text_input(
            ...
            type="password",
            value="",
            placeholder="Enter DASHSCOPE_API_KEY",
            help="The key is only stored in this local process for the current run.",
        )
        if provider_key:
            os.environ["DASHSCOPE_API_KEY"] = provider_key
    ...
    with st.expander("Erweiterte Einstellungen"):
        ...
        use_local_embeddings = st.checkbox(
            "Lokale Embeddings nutzen (Hash)",
            value=True,
            disabled=True,
            help="Wettbewerbsmodus nutzt lokale Hash-Embeddings ohne externe Abhaengigkeiten.",
        )
...
with setup_tab:
    ...
    if input_mode == "Lokales Verzeichnis":
        ...
        uploaded_files = st.file_uploader(
            "Dokumente hochladen (PDF, Word, Excel, JSON/YAML, Bilder)",
        ...
        if uploaded_files:
            if st.button("Dateien für Prüfung vorbereiten"):
                ...
                for subdir in ["pdfs", "excel", "interviews", "screenshots", "logs", "docs"]:
    ...
    provider_key_ok = bool(provider_key or dashscope_key_from_env)
